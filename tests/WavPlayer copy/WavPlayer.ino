...
// Simple 1st order shelving filter state
...
// EQ Configuration: "Universal Headphone" 10-Band Profile
// Band 0 (31Hz) : +1 dB  -> Handled by Bass Shelf
// Band 1 (62Hz) : +1 dB  -> Handled by Bass Shelf
// Band 2 (125Hz):  0 dB  -> Flat Mids
// Band 3 (250Hz):  0 dB     ...
// Band 4 (500Hz):  0 dB     ...
// Band 5 (1kHz) : +1 dB  -> Handled by High Shelf
// Band 6 (2kHz) : +1 dB     ...
// Band 7 (4kHz) : +1 dB     ...
// Band 8 (8kHz) : +1 dB     ...
// Band 9 (16kHz): +1 dB     ...

const float BASS_G = 1.14; // +1.13dB approx
const float TREB_G = 1.14; 

int16_t applyEQ(int16_t sample, float& prev_in, float& prev_out, bool highpass) {
    float in = (float)sample;
    float out;
    
    if (highpass) {
        // High Shelf HPF corner ~1kHz for 44.1kHz
        // Alpha approx 0.86 for 1kHz cut-in
        float hp = 0.86f * (in - prev_in) + 0.72f * prev_out; 
        out = in + (TREB_G - 1.0f) * hp;
        prev_out = hp;
    } else {
        // Low Shelf LPF corner ~100Hz for 44.1kHz
        // Alpha approx 0.014 for 100Hz cut-off
        float lp = 0.014f * in + 0.986f * prev_out;
        out = in + (BASS_G - 1.0f) * lp;
        prev_out = lp;
    }
    prev_in = in;
...
void processAudioBufferEQ(uint8_t* buffer, int bytes) {
    int16_t* samples = (int16_t*)buffer;
    int count = bytes / 2;
    
    for (int i = 0; i < count; i+=2) { // Stereo
       // Left
       samples[i] = applyEQ(applyEQ(samples[i], bass_l_prev_in, bass_l_prev_out, false), treb_l_prev_in, treb_l_prev_out, true);
       // Right
       if (i+1 < count) {
           samples[i+1] = applyEQ(applyEQ(samples[i+1], bass_r_prev_in, bass_r_prev_out, false), treb_r_prev_in, treb_r_prev_out, true);
       }
    }
}

// BackgroundAudio Objects
ESP32I2SAudio *audioOut = NULL;
...
void audioPlaybackTask(void* parameter) {
  ...
  if (!audioOut) {
    audioOut = new ESP32I2SAudio(I2S_BCK, I2S_WS, I2S_DATA);
    Serial.println("✅ ESP32I2SAudio initialized");
  }
  ...
  while (true) {
    ...
    if (needNewFile) {
      ...
      // Re-init I2S for this track
      audioOut = new ESP32I2SAudio(I2S_BCK, I2S_WS, I2S_DATA);
      // Wait briefly for I2S to stabilize
      delay(50);
    ...
    if (audioFile && (mp3Decoder || wavDecoder)) {
      ...
      if (readyForData) {
        ...
        if (bytesRead > 0) {
          ...
          // DSP Application (Safe Mode)
          // Only apply EQ to WAV PCM data, skipping the 44-byte header.
          // MP3 data is compressed here, so we CANNOT apply EQ (it would destroy the stream).
          if (currentFormat == FORMAT_WAV && currentPosition > 44) {
             processAudioBufferEQ(buffer, bytesRead);
          }
